...
import { theme } from '../../logic/client/theme.ts';
import { ThemeEnum } from '../../logic/client/theme-enum.ts';
...
export default function DarkModeToggle(): JSX.Element {
    ...
    return (
        <button type='button' onClick={toggle} class='flex items-center py-4 px-8 transition-colors'>
            ...
            {ThemeEnum.SYSTEM === theme.value && <span class='ml-2'>Auto</span>}
        </button>
    );
}

function toggle(): void {
    theme.value = getNextTheme();
}

function getNextTheme(): ThemeEnum {
    switch (theme.value) {

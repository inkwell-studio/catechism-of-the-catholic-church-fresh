...
import * as dataService from '../client/data-service.ts';
import {
    Language,
    NumberOrNumberRange,
    Paragraph,
...
/*
    ...
    its values are subscribed to through the `Selectors` constant, and
    execution of state-change logic is triggered via the `Actions` constant.
*/
...
async function loadCrossReferenceSelectedContent(paragraphNumbers: NumberOrNumberRange): Promise<void> {
    const paragraphs = await dataService.getParagraphs(paragraphNumbers, state.language.value);

...
Deno.test('all PathIDs are valid', () => {
...
Deno.test('all PathIDs are unique', () => {
    ...
    assertStrictEquals(numPathIDs, numUniquePathIDs, `${numPathIDs - numUniquePathIDs} duplicate pathID values exist`);
});
...
Deno.test('all semanticPaths are unique', () => {
    ...
    assertStrictEquals(numPaths, numUniquePaths, `${numPaths - numUniquePaths} duplicate semanticPath values exist`);
});

Deno.test('all semanticPaths are correctly set relative to their parent', () => {
    const allContent = getAllContent(catechism);
    allContent.forEach((content) => {
...
Deno.test('no semanticPaths should have the number 0 as an identifying number', () => {
    const semanticPaths = getAllSemanticPaths(catechism);

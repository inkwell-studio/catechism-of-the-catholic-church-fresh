...
//#region setup
const englishText = getText(Catechism as CatechismStructure);
...
//#region helpers
function errorMessage(language: string, pathID: PathID, message: string): string {
    return `\n\n[${language}] [${pathID}] ${message}\n`;
}

function testLines(func: (languageParam: string, textParam: string, PathIDParam: PathID) => void): void {
    for (const [language, pathIDsAndText] of allTexts) {
...
function testCharacters(func: (languageParam: string, characterParam: string, pathIDParam: PathID) => void): void {
    for (const [language, pathIDsAndText] of allTexts) {
        for (const { pathID, text } of pathIDsAndText) {
            for (const character of text) {
                func(language, character, pathID);
            }
        }
    }
}
...
function buildInfoText(texts: Array<[string, CatechismText]>): string {
    const textCounts = texts.map(([language, catechismText]) => {
        return {
            language,
            count: catechismText.length,
        };
    });

    const totalCount = textCounts.reduce((count, textCount) => count + textCount.count, 0);
    const textCountsInfo = textCounts
        .map((textCount) => `${textCount.language}: ${textCount.count}`)
        .join(', ');

    return `${totalCount} total — ${textCountsInfo}`;
}
//#endregion

//#region tests
const infoText = buildInfoText(allTexts);
console.log(`\nAll text fragments ... (${infoText}) ...`);
